...
def validate_instruction(line, symbol_table=None, max_address=15):
    ...
    if instr not in instruction_set:
        return False, f"Unknown instruction: {instr}"
    operand_type = instruction_set[instr]["operand"]
    operand = operand1 or operand2
    if operand_type == "none" and operand:
        return False, f"Instruction {instr} takes no operand"
    ...
    if operand_type in ["address", "value"]:
        ...
        try:
            ...
            if value > max_address:
                return False, f"Operand out of range (max {max_address}): {operand}"
            return True, None
        except ValueError:
...
def live_mode():
    ...
    while True:
        try:
            ...
            if not is_valid:
                print(f"Error: {error}")
                action = input("Re-enter line (r), skip (s), or assemble (a)? ").lower()
                if action == "r":
                    continue
...
def main():
    ...
    parser.add_argument("-a", "--no-listing", action="store_false", dest="generate_listing", help="Disable listing file generation")
